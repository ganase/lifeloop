...
| `docs/lifeloop/privacy.html` | App Store ConnectのPrivacy Policy URLに指定するlifeloop個別方針。 |
| `docs/lifeloop/terms.html` | lifeloop個別利用規約。 |
| `docs/lifeloop/support.html` | App Store ConnectのSupport URLに指定するlifeloopサポートページ。 |
| `docs/app-store-submission.md` | App Privacy回答、Review Notes、公開前チェックの控え。 |

会社ホームページで公開する場合は、`docs/` の内容をサイトルートへ配置してください。
